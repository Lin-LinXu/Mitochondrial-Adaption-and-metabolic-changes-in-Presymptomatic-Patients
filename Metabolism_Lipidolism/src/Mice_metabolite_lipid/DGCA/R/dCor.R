...
#' @export
dCorrs <- function(rho1, n1, rho2, n2, corrType = "pearson", var1, var2){

	if(!(all.equal(length(rho1), length(n1), length(rho2), length(n2),length(var1),length(var2)))) stop("All of the input vectors must be the same length.")

	zr1 = atanh(rho1)
	zr2 = atanh(rho2)

	if(corrType == "pearson"){
		diff12 = (zr2 - zr1)/sqrt(abs(var1 + var2))
	}

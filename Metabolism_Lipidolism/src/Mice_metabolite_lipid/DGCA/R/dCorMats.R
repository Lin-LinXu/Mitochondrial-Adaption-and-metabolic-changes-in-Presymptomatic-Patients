...
dCorMats <- function(matA, nmatA, matB, nmatB,
	corr_cutoff = 0.99, corrType = "pearson", secondMat = FALSE, signType = "none",Permut_Variance_lst){
	...
	if(!secondMat){
		...
		Perm_varA = Permut_Variance_lst$Variance_1[upper.tri(Permut_Variance_lst$Variance_1)]
		Perm_varB = Permut_Variance_lst$Variance_2[upper.tri(Permut_Variance_lst$Variance_2)]

		dcorr_res = dCorrs(as.vector(matA), as.vector(nmatA),
			as.vector(matB), as.vector(nmatB), corrType = corrType,
			as.vector(Perm_varA),as.vector(Perm_varB))
